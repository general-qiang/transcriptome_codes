...
# Process DE data
DE_brainpos = defaultdict(dict)

with open('DE_ic_sens_brainpos.csv', 'r') as csvfile:
    ...
    for row_num, row in enumerate(csvreader, 1):
        ...
        try:
            DE_brainpos[gene_id]['logFC'] = float(logFC_str)
            DE_brainpos[gene_id]['pval'] = float(pval_str)
            cleaned_ids = []
            for s in subject_ids.split(','):
                cleaned_id = clean_uniprot_id(s.strip())
                cleaned_ids.append(cleaned_id)
            DE_brainpos[gene_id]['subject_ids'] = [sid for sid in cleaned_ids if sid]
...
for gene_id in DE_brainpos:
    DE_brainpos[gene_id]['blast_results'] = []
    DE_brainpos[gene_id]['uniprot_annotations'] = []
    
    for subject_id in DE_brainpos[gene_id]['subject_ids']:
        ...
        if subject_id in blast_results:
            ...
            DE_brainpos[gene_id]['blast_results'].append(blast_index_value)
        else:
            print(f"No BLAST result for subject ID: {subject_id}")  # Debugging
            DE_brainpos[gene_id]['blast_results'].append(None)
        
        # Add UniProt annotations from local database
        annotations = get_uniprot_annotations(subject_id)
        DE_brainpos[gene_id]['uniprot_annotations'].append(annotations)

# Debugging UniProt annotations
for gene_id in DE_brainpos:
    for i, subject_id in enumerate(DE_brainpos[gene_id]['subject_ids']):
        annotations = DE_brainpos[gene_id]['uniprot_annotations'][i]
        if not annotations:
...
for gene_id in DE_brainpos:
    for i, subject_id in enumerate(DE_brainpos[gene_id]['subject_ids']):
        annotations = DE_brainpos[gene_id]['uniprot_annotations'][i]
        blast_index_value = DE_brainpos[gene_id]['blast_results'][i]
        # print(f"Gene ID: {gene_id}, Subject ID: {subject_id}, BLAST Index: {blast_index_value}")  # Debugging
        if annotations:
            output_data.append({
                'gene_id': gene_id,
                'subject_id': subject_id,
                'logFC': DE_brainpos[gene_id]['logFC'],
                'pval': DE_brainpos[gene_id]['pval'],
...
with open('sens_brainpos_complete_chart.csv', 'w', newline='') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=output_fields)
    writer.writeheader()
    writer.writerows(output_data)

print("Processing complete. Results saved to sens_brainpos_complete_chart.csv.")